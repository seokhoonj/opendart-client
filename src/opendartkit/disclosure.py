...
class Disclosure:
    ...
    def search(
        ...
        *,
        ...
        Auto-paginates: requests ``page_count`` (max 100) rows per page and walks
        every page through ``total_page``, so the result is complete by default --
        no silent first-page-only truncation. ``max_pages`` caps the fetch
        *explicitly* when you want a ceiling. ``begin_date`` / ``end_date`` are
        ``YYYYMMDD``; without ``corp_code`` the DART window is limited to 3 months.
        """
    ...
    def document(self, rcept_no: str) -> bytes:
        ...
        return self._session.get_bytes(DOCUMENT, rcept_no=rcept_no)

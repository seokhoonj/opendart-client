...
(``pip install 'opendartkit[pandas]'`` / ``[polars]``). Neither import runs unless
...
def to_pandas(rows: Rows) -> pd.DataFrame:
    ...
    try:
        ...
        raise ImportError(
            "to_pandas needs pandas -- install with: pip install 'opendartkit[pandas]'"
        ) from err
...
def to_polars(rows: Rows) -> pl.DataFrame:
    ...
    try:
        ...
        raise ImportError(
            "to_polars needs polars -- install with: pip install 'opendartkit[polars]'"
        ) from err
